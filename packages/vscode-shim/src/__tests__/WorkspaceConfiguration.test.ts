...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("jxmh-code-chinese", context)
			...
			setRuntimeConfig("jxmh-code-chinese", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("jxmh-code-chinese", "testSetting", "testValue")

			expect(getRuntimeConfig("jxmh-code-chinese.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("jxmh-code-chinese", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("jxmh-code-chinese.setting1")).toBe("value1")
			expect(getRuntimeConfig("jxmh-code-chinese.setting2")).toBe(42)
			expect(getRuntimeConfig("jxmh-code-chinese.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("jxmh-code-chinese.defined")).toBe("value")
			expect(getRuntimeConfig("jxmh-code-chinese.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("jxmh-code-chinese", "setting1", "value1")
			setRuntimeConfig("jxmh-code-chinese", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("jxmh-code-chinese.setting1")).toBeUndefined()
			expect(getRuntimeConfig("jxmh-code-chinese.setting2")).toBeUndefined()
		})
